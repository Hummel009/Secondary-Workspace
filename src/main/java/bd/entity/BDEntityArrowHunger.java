...
public class BDEntityArrowHunger extends EntityArrow implements IEntityAdditionalSpawnData {
	public BDEntityArrowHunger(World world) {
		super(world);
	}

	public BDEntityArrowHunger(World world, double d, double d1, double d2) {
		super(world, d, d1, d2);
	}

	public BDEntityArrowHunger(World world, EntityLivingBase shooter, EntityLivingBase target, float charge, float inaccuracy) {
		super(world, shooter, target, charge, inaccuracy);
	}

	public BDEntityArrowHunger(World world, EntityLivingBase shooter, float charge) {
	...
	public void onCollideWithPlayer(EntityPlayer entityplayer) {
		...
		if (!worldObj.isRemote && isInGround && arrowShake <= 0) {
			...
			if (canBePickedUp == 1 && !entityplayer.inventory.addItemStackToInventory(new ItemStack(BDRegistry.arrow_hunger, 1))) {
				pickup = false;
			}
			if (pickup) {
