...
public class BDEntityArrowFire extends EntityArrow implements IEntityAdditionalSpawnData {
	public BDEntityArrowFire(World world) {
		super(world);
	}

	public BDEntityArrowFire(World world, double d, double d1, double d2) {
		super(world, d, d1, d2);
	}

	public BDEntityArrowFire(World world, EntityLivingBase shooter, EntityLivingBase target, float charge, float inaccuracy) {
		super(world, shooter, target, charge, inaccuracy);
	}

	public BDEntityArrowFire(World world, EntityLivingBase shooter, float charge) {
	...
	public void onCollideWithPlayer(EntityPlayer entityplayer) {
		...
		if (!worldObj.isRemote && isInGround && arrowShake <= 0) {
			...
			if (canBePickedUp == 1 && !entityplayer.inventory.addItemStackToInventory(new ItemStack(BDRegistry.arrow_fire, 1))) {
				pickup = false;
			}
			if (pickup) {
