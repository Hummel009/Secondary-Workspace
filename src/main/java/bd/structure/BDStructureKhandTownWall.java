...
public class BDStructureKhandTownWall extends BDStructureKhandStructure {
	private boolean isTall = false;

	public BDStructureKhandTownWall(boolean flag) {
		super(flag);
	}
	...
	public boolean generateWithSetRotation(World world, Random random, int i, int j, int k, int rotation) {
		...
		for (j1 = 1; (((j1 >= 0) || !isOpaque(world, 0, j1, 0)) && (getY(j1) >= 0)); --j1) {
			if (random.nextBoolean()) {
				setBlockAndMetadata(world, 0, j1, 0, WOTRMCNewBlocks.khandbrick, 0);
			} else {
		...
		if (isTall) {
			...
			placeWallBanner(world, 0, 7, 0, LOTRItemBanner.BannerType.KHAND, 2);
		} else {
