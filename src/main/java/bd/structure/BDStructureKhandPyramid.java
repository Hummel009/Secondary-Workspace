...
public class BDStructureKhandPyramid extends BDStructureKhandStructure {
	public BDStructureKhandPyramid(boolean flag) {
		super(flag);
	}
	...
	public boolean generateWithSetRotation(World world, Random random, int i, int j, int k, int rotation) {
		...
		loadStrScan("khand_pyramid");
		associateBlockMetaAlias("STONE", WOTRMCNewBlocks.khandbrick, 0);
		...
		addBlockMetaAliasOption("BRICK", 8, WOTRMCNewBlocks.khandbrickaligned, 0);
		...
		for (i1 = -5; i1 <= 5; ++i1) {
			for (k1 = -5; k1 <= 5; ++k1) {
				...
				this.placeChest(world, random, i1, j13, k1, LOTRMod.chestBasket, MathHelper.getRandomIntegerInRange(random, 2, 5), BDChestContents.KHAND_HOUSE);
			}
		}
		int maxStep = 4;
		for (int k12 : new int[] { -11, 11 }) {
			...
			for (step = 0; step < maxStep && !isOpaque(world, i12 = -7 - step, j1 = 0 - step, k12); ++step) {
				...
				while (!isOpaque(world, i12, j2, k12) && getY(j2) >= 0) {
					setBlockAndMetadata(world, i12, j2, k12, WOTRMCNewBlocks.khandbrick, 0);
		...
		for (int i13 : new int[] { -11, 11 }) {
			...
			for (step = 0; step < maxStep && !isOpaque(world, i13, j1 = 0 - step, k13 = -7 - step); ++step) {
				...
				while (!isOpaque(world, i13, j2, k13) && getY(j2) >= 0) {
					setBlockAndMetadata(world, i13, j2, k13, WOTRMCNewBlocks.khandbrick, 0);
