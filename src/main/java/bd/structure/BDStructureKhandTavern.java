...
public class BDStructureKhandTavern extends BDStructureKhandStructure {
	public BDStructureKhandTavern(boolean flag) {
		super(flag);
	}
	...
	public boolean generateWithSetRotation(World world, Random random, int i, int j, int k, int rotation) {
		...
		placeKebabStand(world, random, 2, 2, 2, LOTRMod.kebabStand, 2);
		placeWallBanner(world, -2, 4, -3, LOTRItemBanner.BannerType.KHAND, 2);
		placeWallBanner(world, 2, 4, -3, LOTRItemBanner.BannerType.KHAND, 2);
		placeWallBanner(world, -2, 4, 3, LOTRItemBanner.BannerType.KHAND, 0);
		placeWallBanner(world, 2, 4, 3, LOTRItemBanner.BannerType.KHAND, 0);
		...
		BDEntityKhandBartender bartender = new BDEntityKhandBartender(world);
		...
		for (int l = 0; l < numHaradrim; ++l) {
			BDEntityKhandMan haradrim = new BDEntityKhandMan(world);
		...
		for (i1 = -1; i1 <= 1; ++i1) {
			for (step = 0; step < 12 && !isOpaque(world, i1, j1 = 0 - step, k1 = 10 + step); ++step) {
				setBlockAndMetadata(world, i1, j1, k1, LOTRMod.stairsRedSandstone, 3);
				setGrassToDirt(world, i1, j1 - 1, k1);
				j2 = j1 - 1;
				while (!isOpaque(world, i1, j2, k1) && getY(j2) >= 0) {
					setBlockAndMetadata(world, i1, j2, k1, LOTRMod.redSandstone, 0);
