...
import bd.database.*;
import bd.entity.BDEntityKhandBlacksmith;
import lotr.common.*;
import net.minecraft.init.Blocks;
import net.minecraft.item.ItemStack;
import net.minecraft.world.World;

public class BDStructureKhandSmithy extends BDStructureKhandStructure {
	public BDStructureKhandSmithy(boolean flag) {
		super(flag);
	}
	...
	public boolean generateWithSetRotation(World world, Random random, int i, int j, int k, int rotation) {
		...
		this.placeChest(world, random, -4, 1, -2, LOTRMod.chestBasket, 3, BDChestContents.KHAND_HOUSE);
		...
		if (random.nextBoolean()) {
			placeArmorStand(world, -7, 1, -2, 1, new ItemStack[] { null, new ItemStack(BDRegistry.khand1_chestplate), new ItemStack(BDRegistry.khand1_leggings), new ItemStack(BDRegistry.khand1_boots) });
		} else {
			placeArmorStand(world, -7, 1, -2, 1, new ItemStack[] { null, new ItemStack(BDRegistry.khand1_chestplate), null, null });
		}
		placeWeaponRack(world, -13, 3, 0, 5, getRandomKhandWeapon(random));
		BDEntityKhandBlacksmith smith = new BDEntityKhandBlacksmith(world);
