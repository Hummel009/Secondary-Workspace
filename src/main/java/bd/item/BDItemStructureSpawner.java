...
public class BDItemStructureSpawner extends Item {
	...
	public int getColorFromItemStack(ItemStack itemstack, int pass) {
		BDStructure.StructureColorInfo info = BDStructure.structureItemSpawners.get(itemstack.getItemDamage());
		if (info != null) {
			if (pass == 0) {
	...
	public IIcon getIconFromDamageForRenderPass(int i, int pass) {
		BDStructure.StructureColorInfo info = BDStructure.structureItemSpawners.get(i);
		if (info != null) {
			if (info.isVillage) {
				if (pass == 0) {
	...
	public String getItemStackDisplayName(ItemStack itemstack) {
		...
		String structureName = BDStructure.getNameFromID(itemstack.getItemDamage());
	...
	public void getSubItems(Item item, CreativeTabs tab, List list) {
		for (BDStructure.StructureColorInfo info : BDStructure.structureItemSpawners.values()) {
			if (info.isHidden) {
				continue;
			}
	...
	private boolean spawnStructure(EntityPlayer entityplayer, World world, int id, int i, int j, int k) {
		if (!BDStructure.structureItemSpawners.containsKey(id)) {
			return false;
		}
		BDStructure.IStructureProvider strProvider = BDStructure.getStructureForID(id);
		if (strProvider != null) {
