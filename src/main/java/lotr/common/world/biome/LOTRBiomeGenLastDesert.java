...
public class LOTRBiomeGenLastDesert extends LOTRBiome {
	...
	public void decorate(World world, Random random, int i, int k) {
		...
		if (random.nextInt(8) == 0) {
			...
			getRandomWorldGenForGrass(random).generate(world, random, i1, j1, k12);
		}
		...
		if (random.nextInt(20) == 0) {
			...
			k12 = k + random.nextInt(16) + 8;
			j1 = world.getHeightValue(i1, k12);
			new WorldGenDeadBush(Blocks.deadbush).generate(world, random, i1, j1, k12);
		}
		...
		if (random.nextInt(500) == 0) {
			...
			for (l = 0; l < trees; ++l) {
				...
				decorator.genTree(world, random, i12, j12, k1);
			}
		}
	}
